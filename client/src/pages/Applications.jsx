...
const Applications = () => {
      ...
      if (selectedJob) {
        try {
          const res = await axios.get(
            `http://localhost:8080/jobApp/appliedCt/${selectedJob.id}`
          );
          if (res.status === 200) {
  ...
  const handleApply = async () => {
    try {
      const res = await axios.post(`http://localhost:8080/jobApp/addJobApp`, {
